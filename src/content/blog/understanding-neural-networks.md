...
Training neural networks involves numerous practical decisions that significantly affect performance. **Overfitting** occurs when a network memorizes the training data rather than learning general patterns, performing well on training examples but poorly on new data. [Machine learning techniques](/blog/ml-vs-deep-learning) to combat overfitting include dropout (randomly deactivating neurons during training), regularization (penalizing large weights), and data augmentation (creating variations of training examples).
